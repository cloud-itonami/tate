#!/usr/bin/env bb
;; tate 盾 — legal-defense DATOM-LOG writer (content-addressed commit-DAG).
(ns tate.methods.kotoba
  "kotoba.cljc — tate 盾 append-only DEFENSE LEDGER writer
  (on ADR-2605262130 + ADR-2605312345; same content-addressed commit-DAG
  machinery as the ugachi/busshi/meisai family — the datomic-isomorphic
  canonical-state layer the legal actors previously only EMITTED but never
  PERSISTED).

  Persists tate's GROUND datoms — the coded clause-pattern + procedure
  registries and the member's own docs/notices (synthetic at R0, G1) — to a
  local append-only EDN log of content-addressed transactions. DERIVED state
  (clause flags + plan status) is NEVER stored: it is computed on READ
  (datom_emit :bond/is-transient, G2), so this ledger holds only durable
  ground facts.
   ...
   (do
     (defn append-tx [tx log-path]
       (let [f (io/file log-path)]
         ...
         (when-not (.exists f)
           (spit f (str ";; tate 盾 — DEFENSE LEDGER (append-only, content-addressed "
                        "EAVT commit-DAG of the member's coded registries + own docs/notices). "
                        "Generated; DO NOT hand-edit. A record of legal POSTURE, never advice "
                        "and never a verdict. ADR-2605312345.\n")))
