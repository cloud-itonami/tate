(ns tate.methods.social
  "social.cljc — tate DRY-RUN self-publication seed (ADR-2606272355, auto-seeded R0).

  This is a GENERIC self-publication projection (draft-observation-post below builds an
  app.bsky.feed.post-shaped record), mechanically derived from tate's own
...
(defn draft-observation-post
  "A dry-run observation post over tate's own domain (Citizen legal-defense CONCIERGE (盾 = shield; defensive only) — two legs over the member's OWN paper: (1) 不利条項スキャン: the member's consumer…)."
...
(defn build-live
  ...
  [& _args]
  (throw (ex-info (str "tate R0: live social posting is Council Lv6+ + operator + "
